...
public class Principale {
    ...
    public static void main(String[] args) {
        ...
        xmlContact.create(c1);

        c1.setEmailContact("email@gmail.com");
        XMLContactDAO.getInstance().update(c1);
        
        xmlContact.ajouter(c2);

        //  new VuePrincipale();
        ArrayList<Manifestation> listeManif = new ArrayList<Manifestation>();
        listeManif = xmlManif.readAll();

        for (int i = 0; i < listeManif.size(); i++) {
            System.out.println(listeManif.get(i));
        }
        
        ArrayList<Contact> listeContact = new ArrayList<Contact>();
        listeContact = xmlContact.readAll();
